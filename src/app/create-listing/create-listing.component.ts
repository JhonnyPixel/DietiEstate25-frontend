...
export class CreateListingComponent implements OnInit {
    ...
    if (this.propertyForm.valid) {
      ...
      if (this.isEditMode) {
        ...
        this.restBackend.createListing(this.propertyForm.value).subscribe((data:any) => {
          ...
          // Qui potresti aggiungere una notifica di successo e redirect
          console.log("photos:",this.propertyForm.value.photos)
          this.restBackend.uploadListingImages(data.id, this.propertyForm.get('photos')!.value!).subscribe(
            data=>{
              console.log(data)
            }
          )
  ...
  onPhotoUpload(event: any) {
      ...
      if (files) {
        ...
        // Aggiungi i file al formControl 'photos'
        const currentPhotos = this.propertyForm.get('photos')!.value || [];
        const updatedPhotos = [...currentPhotos, ...fileList];
        this.propertyForm.get('photos')!.setValue(updatedPhotos);
      }
  }
  ...
  export class CreateListingComponent implements OnInit {
    ...
    photosToDelete: number[] = []; // IDs delle foto esistenti da eliminare
        ...
        if (this.isEditMode) {
          ...
          this.restBackend.updateListing(listingId, this.propertyForm.value).subscribe(data => {
            ...
            if (this.photosToDelete.length > 0) {
              console.log("Deleting photos:", this.photosToDelete);

              //const paths=this.photosToDelete.map((index)=>{this.photoPreviews[]})
              this.restBackend.deleteListingImages(listingId, this.photosToDelete).subscribe(
                deleteRes => {
    ...
    removePhoto(index: number) {
      ...
      // Se è una foto esistente (non nuova), aggiungi l'ID all'elenco delle foto da eliminare
      if (!photoToRemove.isNew && photoToRemove.id !== undefined) {
    ...
    clearAllPhotos() {
      // Se ci sono foto esistenti, aggiungile tutte all'elenco delle foto da eliminare
      if (this.isEditMode) {
        const existingPhotos = this.photoPreviews.filter(p => !p.isNew && p.id !== undefined);
